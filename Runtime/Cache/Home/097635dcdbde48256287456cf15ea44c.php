...
<div class="navbar navbar-inverse navbar-fixed-top">
	...
	<div class="cf">
		<div class="fl" style="float:right">
			<a class="btn btn-success" style="margin-right:15px" href="<?php echo U('bianji2',array('id'=>$a['xiangmu_id'],'name'=>$a['name'],'fenlei'=>$a['fenlei'],'man'=>$a['man'],'company'=>$a['company'],'location'=>$a['location'],'phone'=>$a['phone'],'work_progress'=>$a['work_progress'],'next_plan'=>$a['next_plan'],'problem'=>$a['problem']));?>">新建更新状态</a>
                <a class="btn btn-success"  style="margin-right:15px" href="<?php echo U('dayin_current',array('xiangmu_id'=>$a['xiangmu_id'],'name'=>$a['name'],'fenlei'=>$a['fenlei'],'man'=>$a['man'],'company'=>$a['company'],'location'=>$a['location'],'phone'=>$a['phone'],'work_progress'=>$a['work_progress'],'next_plan'=>$a['next_plan'],'problem'=>$a['problem']));?>">打印当前记录</a>
			<?php if(is_array($b)): $i = 0; $__LIST__ = $b;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$b): $mod = ($i % 2 );++$i;?><a   class="btn btn-success " type="button"
			    href="<?php echo U('Index/bianji3',array('id'=>$b['id'],'name'=>$b['name'],'fenlei'=>$b['fenlei'],'man'=>$b['man'],'company'=>$b['company'],'location'=>$b['location'],'phone'=>$b['phone'],'work_progress'=>$b['work_progress'],'next_plan'=>$b['next_plan'],'problem'=>$b['problem'],'user_uid'=>$b['user_uid']));?>">编辑 </a><?php endforeach; endif; else: echo "" ;endif; ?>	
      	</div>
		<div>
			<form action="<?php echo U('Index/sousuo_xiangmu_copy');?>" method="post">
				...
				<input type="hidden" name="xiangmu_id" value="<?php echo ($_GET['id']); ?>" />  
        ...
        <table class="table table-bordered">
			...
			<tbody>
				<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
							...
							<td><?php echo ($vo["time"]); ?></td><?php endforeach; endif; else: echo "" ;endif; ?>		
